...
const schema = {"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"address":{"type":"string"}},"required":["name"]}
// Injected endpoints configuration for conditional actions
const endpoints = {"GET":{"operationId":"getEditorials","summary":"Obtener lista de editoriales","responses":{"200":{"description":"Lista de editoriales","content":{"application/json":{"schema":{"type":"array","items":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"address":{"type":"string"}},"required":["name"]}}}}}}},"POST":{"operationId":"createEditorial","summary":"Crear una nueva editorial","requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"address":{"type":"string"}},"required":["name"]}}}},"responses":{"201":{"description":"Editorial creada"}}}}
...
const editorials = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/editorials`, { headers })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteConfirmed = () => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/editorials/${deleteId}`, {
      method: 'DELETE',
      headers
    })
      .then((res) => {
        if (res.ok) {
          ...
          toast({
            ...
            content: 'The new editorials has been successfully deleted.',
  ...
  return (
    <div className="container-fluid p-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h3 mb-0">editorials List</h1>
        {endpoints.POST && (
          <Button
            color="primary"
            iconLeft="plus"
            label="Add New"
            onClick={() => navigate(`/editorials/create`)}
          />
        )}
      </div>
      ...
      <DynamicTable
        data={currentItems}
        schema={schema}
        onView={(item) => navigate(`/editorials/${item.id}`)}
        onEdit={(item) => navigate(`/editorials/edit/${item.id}`)}
