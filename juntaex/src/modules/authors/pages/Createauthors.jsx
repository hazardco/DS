...
const Createauthors = () => {
  ...
  const onSubmit = (values) => {
    ...
    fetch(`${import.meta.env.VITE_SERVER_URL}/authors`, {
      method: 'POST', headers, body: JSON.stringify(payload)
    })
      .then(res => {
        if (!res.ok) throw new Error('Error submitting form')
        // Show success toast and navigate to list
        toast({ status: 'success', title: 'Submission successful', content: 'The new authors has been successfully created.', button: { label: 'Go to list', iconRight: 'arrow-right' }, duration: 2000 })
        navigate('/authors')
      })
  ...
  return (
    <FormContainer title={`Create authors`}>
