...
const schema = {"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"surname":{"type":"string"},"phone":{"type":"string"}},"required":["name","surname","phone"]}
// Injected endpoints configuration for conditional actions
const endpoints = {"GET":{"operationId":"getAuthors","summary":"Obtener lista de autores","responses":{"200":{"description":"Lista de autores","content":{"application/json":{"schema":{"type":"array","items":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"surname":{"type":"string"},"phone":{"type":"string"}},"required":["name","surname","phone"]}}}}}}},"POST":{"operationId":"createAuthor","summary":"Crear un nuevo autor","requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"name":{"type":"string"},"surname":{"type":"string"},"phone":{"type":"string"}},"required":["name","surname","phone"]}}}},"responses":{"201":{"description":"Autor creado"}}}}
...
const authors = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/authors`, { headers })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteConfirmed = () => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/authors/${deleteId}`, {
      method: 'DELETE',
      headers
    })
      .then((res) => {
        if (res.ok) {
          ...
          toast({
            ...
            content: 'The new authors has been successfully deleted.',
  ...
  return (
    <div className="container-fluid p-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h3 mb-0">authors List</h1>
        {endpoints.POST && (
          <Button
            color="primary"
            iconLeft="plus"
      ...
      <DynamicTable
        data={currentItems}
        schema={schema}
        onView={(item) => navigate(`/authors/${item.id}`)}
        onEdit={(item) => navigate(`/authors/edit/${item.id}`)}
