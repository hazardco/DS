...
const schema = {"type":"object","properties":{"id":{"type":"integer","readOnly":true},"description":{"type":"string"}},"required":["description"]}
// Injected endpoints configuration for conditional actions
const endpoints = {"GET":{"operationId":"getCategories","summary":"Obtener lista de categorías","responses":{"200":{"description":"Lista de categorías","content":{"application/json":{"schema":{"type":"array","items":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"description":{"type":"string"}},"required":["description"]}}}}}}},"POST":{"operationId":"createCategory","summary":"Crear una nueva categoría","requestBody":{"required":true,"content":{"application/json":{"schema":{"type":"object","properties":{"id":{"type":"integer","readOnly":true},"description":{"type":"string"}},"required":["description"]}}}},"responses":{"201":{"description":"Categoría creada"}}}}
...
const categories = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/categories`, { headers })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteConfirmed = () => {
    fetch(`${import.meta.env.VITE_SERVER_URL}/categories/${deleteId}`, {
      method: 'DELETE',
      headers
    })
      .then((res) => {
        if (res.ok) {
          ...
          toast({
            ...
            content: 'The new categories has been successfully deleted.',
  ...
  return (
    <div className="container-fluid p-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h1 className="h3 mb-0">categories List</h1>
        {endpoints.POST && (
          <Button
            color="primary"
            iconLeft="plus"
      ...
      <DynamicTable
        data={currentItems}
        schema={schema}
        onView={(item) => navigate(`/categories/${item.id}`)}
        onEdit={(item) => navigate(`/categories/edit/${item.id}`)}
